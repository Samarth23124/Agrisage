...
class Location extends React.Component {
  ...
  onSubmit = (e) => {
    const url = `/soil/latest/by-lat-lng?lat=${this.state.lat}&lng=${this.state.lng}`;
    axios({
      method: 'GET',
      url: url,
      headers: {
        'x-api-key':
          '61a253402670cbee70655a556c7cdc5e0660f28e115cfd0ef8a15a932db8f4f7',
      },
    })
      .then((response) => {
        this.setState({
  ...
  onSubmitFire = (e) => {
    const url = `http://localhost:5000/api/fire?lat=${this.state.lat}&lng=${this.state.lng}`;
    axios({
      method: 'GET',
      url: url,
      headers: {
  ...
  onSubmitPollen = (e) => {
    const url = `http://localhost:5000/api/pollen?lat=${this.state.lat}&lng=${this.state.lng}`;
    axios({
      method: 'GET',
      url: url,
      headers: {
  ...
  onSubmitWater = (e) => {
    const url = `http://localhost:5000/api/weather?lat=${this.state.lat}&lng=${this.state.lng}`;
  ...
  onSubmitAir = (e) => {
    const url = `http://localhost:5000/api/air?lat=${this.state.lat}&lng=${this.state.lng}`;
